...
Task B examined 50 cases from the ChemRxiv evaluation set, evenly split between 25 cases in which the designated gold passage appeared in the top 10 results and 25 cases in which it did not. The reviewer saw the query, its designated gold passage, and the top 10 passages retrieved by \texttt{ChEmbed\textsubscript{progressive}}. In addition to the Task A criteria, each non-gold passage was classified as not relevant, somewhat relevant, or relevant. The reviewer could also identify near misses, defined as non-gold passages that were nevertheless strongly relevant to the same information need.

For Task A, 48 of 50 queries (96\%) were judged answerable. The mean ratings were 4.76 for specificity, 4.56 for query quality, 4.92 for standalone clarity, and 4.72 for scientific validity. For Task B, 45 of 50 queries (90\%) were judged answerable, with corresponding mean ratings of 4.98, 4.84, 5.00, and 5.00. At least one near miss was identified in 32 of the 50 retrieval cases. Because Task B deliberately contained equal numbers of successful and unsuccessful retrieval cases, the 32-of-50 near-miss count characterizes this balanced review sample and should not be interpreted as a 64\% near-miss rate for the full benchmark.
...
Open-domain Retrieval & MTEB Retrieval & \xmark & 10 & 1482 & $109,645$ & 0.544 & 0.467 & 0.475 \\
\bottomrule
...
To demonstrate an example of how this fine-tuning would impact the chemical topics retrieval, we evaluated its impact on chemical role retrieval.
To achieve this goal, combination of PubChem dataset \cite{kim2019pubchem} for compounds description and ChEBI \citep{malik2025chebi} knowledge graph for structured role annotation, were employed.

The hypothesis was whether a natural-language query about a chemical role would retrieve PubChem descriptions of compounds that ChEBI directly assigns to that role more effectively. PubChem supplied the descriptions, while ChEBI supplied structured role annotations independently of the embedding models \cite{kim2019pubchem,malik2025chebi}.
...
With above approach we got a total of 1,554 queries. All 44,887 descriptions, got embedeed with baseline Nomic model, ChEmbed\textsubscript{vanilla}, and ChEmbed\textsubscript{progressive}. Then these embedded descriptions got sorted for each query.
The directly annotated CIDs formed the reference set for that role. Average precision summarizes how early and consistently these reference descriptions appear across the complete ranking, while recall at 10 is the proportion appearing among the first ten results. Table~\ref{tab:chebi-role-retrieval} summarizes the results.
