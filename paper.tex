...
We then asked two questions: whether these two kinds of chemistry text really differ, and what fine-tuning changed. For the first, we encoded 800 ChemRxiv passages and 800 encyclopedic passages with the base model. A logistic-regression classifier told the two groups apart with 95.6\% accuracy, and passage length alone did not account for this. Chemistry literature and chemistry-related encyclopedic text are therefore written differently, even when they cover the same subject. For the second, we measured how close ChemRxiv passages sat to one another before and after fine-tuning. In the base model they were crowded together, with a mean pairwise cosine similarity of 0.570. After fine-tuning this fell to 0.030, so the passages had spread out and become easier to tell apart. The spreading was stronger for chemistry literature than for encyclopedic text \cite{ethayarajh2019contextual}. Both analyses are reported in full in the ESI.
