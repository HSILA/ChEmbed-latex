...
\footnotetext{\dag~Supplementary information (SI): base-model separation of chemistry literature from encyclopedic text, and the change in embedding geometry after fine-tuning. See DOI: 00.0000/00000000.}
...
2   & PubChem descriptions &  Compound descriptions    &   393,321  & Training & \texttt{gpt-4.1-nano} \\
...
5   & ChemRxiv paragraphs        & Extracted ChemRxiv paragraphs (CC-BY-NC) &    69,457  & Evaluation & \texttt{Claude 3.7 Sonnet} \\
...
\subsection{Expert Review of Synthetic Queries and Retrieval Results}
\label{subsec:expert-review}
To assess whether the generated queries represented meaningful chemistry information needs, we conducted a structured review with one domain expert annotator (with a post-graduate degree in Chemistry). The review comprised two tasks and used the interface shown in Fig.~\ref{fig:expert-review}. Task A covered 50 training query-passage pairs, with 25 from ChemRxiv and 25 from the Dolma chemistry collection. For each pair, the reviewer judged whether the source passage answered the query and assigned ratings from 1 (low) to 5 (high) for specificity, query quality, standalone clarity, and scientific validity. Specificity reflected how closely the query was tied to the passage; query quality, its naturalness and usefulness as a search query; standalone clarity, whether it could be understood without additional context; and scientific validity, its faithfulness to the chemistry in the passage. An optional free-text note was also available.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\linewidth]{figures/expert-ui.png}
    \caption{Expert-review interface for Task A. The reviewer inspected a generated query alongside its real source chemistry passage and recorded answerability, specificity, query quality, standalone clarity, scientific validity, and an optional qualitative note.}
    \label{fig:expert-review}
\end{figure*}

In the evaluation, only the passage used to generate each query was considered relevant, and MRR was calculated from the rank of that passage. To evaluate whether other correct documents might exist for the queries, a second task was defined.
In this task, we selected 50 cases from the ChemRxiv evaluation set, evenly split between 25 cases in which the designated gold passage appeared in the top 10 results and 25 cases in which it did not. The reviewer saw the query, its designated gold passage, and the top 10 passages retrieved by \texttt{ChEmbed\textsubscript{progressive}}. In addition to evaluating the queries and answers using the Task A criteria, the human expert was asked to classify each non-gold passage as either \textit{not relevant}, \textit{somewhat relevant}, or \textit{relevant}. If a strongly relevant non-gold passage was identified, the reviewer could mark it as a near miss.

For Task A, 48 of 50 queries (96\%) were judged answerable. The mean ratings were 4.76 for specificity, 4.56 for query quality, 4.92 for standalone clarity, and 4.72 for scientific validity. For Task B, 45 of 50 queries (90\%) were judged answerable, with corresponding mean ratings of 4.98, 4.84, 5.00, and 5.00. At least one near miss (a strongly relevant non-gold passage) was identified in 32 of the 50 retrieval cases. No item was removed from the evaluation set as a result of this review, and its findings did not feed back into any evaluation.

We used these annotations as a post-hoc human assessment of synthetic-query quality and as a diagnostic complement to the automatic rank-based metrics. The review outcomes were not used to filter or regenerate training pairs, update model parameters, or alter the benchmark relevance labels.

\section{Model Architecture and Domain Adaptation}
A core challenge in building domain-specialized embedding models is choosing a base architecture that is both performant and open for further adaptation. For this work, we selected the nomic embedding family \cite{nussbaum2024nomic}, which represents one of the best and most lightweight open-source text embedding models available, supporting long context up to 8192 tokens. Unlike many proprietary or partially open alternatives, the nomic models provide full access to intermediate weights and pretraining data, enabling reproducibility and deeper scientific analysis. Architecturally, nomic builds on a BERT but introduces key architectural improvements, including rotary positional embeddings, FlashAttention for efficient long-context processing, and SwiGLU activations \cite{shazeer2020glu}, that collectively improve both accuracy and scalability. It is also the best-performing open-source embedding model on the domain-specific chemical benchmark \cite{pmlr-v262-shiraee-kasmaee24a}, and its training on large general corpora already gives it a broad knowledge of chemical language. For these reasons, nomic models serve as an ideal starting point for domain adaptation in chemistry.
...
\paragraph*{Experimental setup} We fine-tune \texttt{nomic-embed-text-v1} for the chemical retrieval task. The fine-tuning is performed with and without adding new tokens to the tokenizer. For the optimized addition of new tokens, we tried three approaches, which are explained in more detail in Section~\ref {subsec:tokenizer} and Table~\ref{tab:tok-ablation}.

Effectiveness is assessed through retrieval tasks using three benchmark suites: ChemRxiv Retrieval, MTEB (English v2), and ChemTEB. {ChemRxiv Retrieval} is our newly developed benchmark and, to our knowledge, the largest retrieval benchmark built directly from chemistry literature paragraphs, comprising a corpus of 69,457 paragraphs from chemical literature published on ChemRxiv and 5,000 synthetic queries. Queries are generated using a Large language model (Anthropic's Claude 3.7 Sonnet) different from the one used for training, thereby mitigating potential generation bias. This task has been integrated into the latest release of ChemTEB. {MTEB (English v2)} \cite{muennighoff2022mteb} is a widely recognized retrieval benchmark featuring 41 English-language tasks across seven categories, including classification, clustering, and retrieval. However, it is a general domain benchmark and does not focus on chemistry use cases. Given that our model is English and the primary goal of this work is to adapt models for retrieval tasks in the chemical domain, we exclusively evaluate the model using its retrieval datasets. \textbf{ChemTEB} \cite{pmlr-v262-shiraee-kasmaee24a} is a chemistry-focused adaptation of MTEB containing two retrieval tasks mainly sourced from encyclopedic data, aiming to evaluate the generalizability of embedding models to chemical domains.
...
\midrule
    ...
    \texttt{nomic-bert-2048} & 768 & 137.3 & 0.019 & 0.025 \\
    ...
    \texttt{bge-small-en} & 384 & 33.4 & 0.705 & 0.751 \\
    \texttt{bge-base-en} & 768 & 109.0 & 0.721 & 0.766 \\
    \texttt{bge-large-en} & 1024 & 335.0 & 0.741 & 0.785 \\
    \texttt{bge-small-en-v1.5} & 384 & 33.4 & 0.704 & 0.750 \\
    ...
    \texttt{stella\_en\_1.5B\_v5} & 1536 & 1540.0 & 0.760 & 0.802 \\
    \texttt{jina-embeddings-v3} & 1024 & 572.0 & 0.715 & 0.759 \\
    \texttt{embeddinggemma-300m} & 768 & 307.6 & 0.799 & 0.836 \\
    \texttt{Qwen3-Embedding-0.6B}$^{a}$ & 1024 & 595.8 & 0.803 & 0.840 \\
    \texttt{Qwen3-Embedding-4B}$^{a}$ & 2560 & 4021.8 & 0.838 & 0.871 \\
    \texttt{Qwen3-Embedding-8B}$^{a}$ & 4096 & 7567.3 & 0.846 & 0.878 \\
    ...
    \bottomrule
\multicolumn{5}{l}{\footnotesize $^{a}$\,Loaded the model with bfloat16 to fit into GPU VRAM.}\\\end{tabular}
\end{table*}
...
\begin{figure*}[!t]
        ...
        \caption{Model efficiency on the ChemRxiv retrieval task. The x-axis represents retrieval speed, the y-axis shows ranking effectiveness measured by Mean Reciprocal Rank ($MRR@10$), and circle sizes represent the number of model parameters.}
        \label{fig:bubble}
\end{figure*}

\paragraph*{Speed-performance trade-off analysis}
Fig.~\ref{fig:bubble} visualizes the efficiency-performance balance of various embedding models on the ChemRxiv retrieval. The models are plotted according to retrieval speed (examples per second) on the horizontal axis and \textit{MRR@10} on the vertical axis. The marker size indicates the model's parameter count, while color reflects the maximum embedding dimension provided by the model. On an NVIDIA A10 24GB GPU, \textbf{ChEmbed\textsubscript{vanilla}} and \textbf{ChEmbed\textsubscript{progressive}} processed approximately 158 and 171 examples per second, respectively (a mean of 165), whereas \texttt{Qwen3-Embedding-4B} and \texttt{Qwen3-Embedding-8B} processed approximately 56 and 38 examples per second, respectively. The two Qwen3 models contain approximately 29 and 55 times as many parameters as ChEmbed, respectively, yet still fall short in retrieval accuracy, with their best variant (\texttt{Qwen3-Embedding-8B}) achieving only 0.846 \textit{MRR@10} compared to ChEmbed's 0.882. This demonstrates that ChEmbed achieves state-of-the-art chemical literature retrieval performance while being significantly faster and more lightweight than its closest open-source alternatives.
...
The retrieval comparison in Table~\ref{tab:retrieval-wide} warrants closer attention. Fine-tuning raises ChemRxiv Retrieval \textit{MRR@10} from 0.781 to 0.879 for ChEmbed\textsubscript{vanilla}. On the two encyclopedic ChemTEB tasks, which are chemistry-related subsets of Natural Questions and HotpotQA \cite{kwiatkowski2019natural,yang2018hotpotqa}, the mean changes from 0.728 to 0.672. The ChemTEB(latest) score averages all three tasks, so it combines the ChemRxiv gain with the encyclopedic losses. The two encyclopedic tasks are also small, with 27 and 18 test queries. A single query moving from first to second place changes MRR@10 by about 0.02 on ChemNQ and 0.03 on ChemHotpotQA. The two sets may differ from ChemRxiv Retrieval in text genre rather than subject matter: their corpora are encyclopedic, whereas ChemRxiv Retrieval contains preprint paragraphs. Because retrieval transfer can depend on corpus distribution as well as topic \cite{thakur2021beir}, we looked at whether the two corpora differ in the unmodified base model.
...
\toprule
 & & \multicolumn{3}{c|}{\textbf{Dataset statistics}} & \multicolumn{3}{c}{\textbf{Retrieval Score$^{b}$ $\uparrow$}} \\
\cmidrule(lr){3-5}\cmidrule(lr){6-8}
Task & Dataset
& \#Tasks & Avg Queries & Avg Corpus & nomic-embed-text-v1 & \texttt{ChEmbed\textsubscript{vanilla}} & \texttt{ChEmbed\textsubscript{progressive}} \\
\midrule
Chemistry Retrieval & ChemTEB(latest)$^{a}$
& 3 & 1682 & $34,153$ & 0.746 & 0.741 & 0.728 \\
Open-domain Retrieval & MTEB Retrieval
& 10 & 1482 & $109,645$ & 0.544 & 0.467 & 0.475 \\
\bottomrule
\multicolumn{8}{l}{\footnotesize $^{a}$\,ChemTEB(latest) comprises ChemRxiv Retrieval and two encyclopedic tasks; individual ChemRxiv Retrieval scores appear in Table~\ref{tab:chemrxiv-results}.}\\
\multicolumn{8}{l}{\footnotesize $^{b}$\,MRR@10 for Chemistry Retrieval; NDCG@10 for Open-domain Retrieval (multiple relevant documents per query). ChemRxiv Retrieval and ChemHotpotQA have one relevant document per query; ChemNQ averages 1.3.}\\
\end{tabular}}
\end{table*}

We then asked two questions: whether these two kinds of chemistry text really differ, and what fine-tuning changed. For the first, we encoded 800 ChemRxiv passages and 800 encyclopedic passages with the base model. A logistic-regression classifier told the two groups apart with 95.6\% accuracy, and passage length alone did not account for this. Chemistry literature and chemistry-related encyclopedic text are therefore represented as measurably different populations by the base model. For the second, we measured how close ChemRxiv passages sat to one another before and after fine-tuning. In the base model they were crowded together, with a mean pairwise cosine similarity of 0.570. After fine-tuning this fell to 0.030, so the passages had spread out, a change consistent with the higher retrieval scores \citep{wang2020understanding}. The spreading was stronger for chemistry literature than for encyclopedic text \cite{ethayarajh2019contextual}. Both analyses are described in detail in the ESI.
...
\subsection{Impact of model fine-tuning on role-based retrieval}
\label{subsec:chebi-role-retrieval}
To demonstrate an example of how this fine-tuning would impact the chemical literature retrieval, we evaluated its impact on a chemical role retrieval task.
To achieve this goal, a combination of the PubChem dataset \cite{kim2019pubchem} for compound descriptions and ChEBI \citep{malik2025chebi} knowledge graph for structured role annotation was used.

The hypothesis was that a natural-language query about a chemical role would retrieve PubChem descriptions of compounds that ChEBI directly assigns to that role more effectively. PubChem supplied the descriptions, while ChEBI supplied structured role annotations independently of the embedding models.

\paragraph*{Task Construction}
For each PubChem CID, we retained one 20 to 500 word description and removed exact duplicate texts yielding 44,887 retrieval candidates. We linked these PubChem records to ChEBI using explicit database links or exact full-InChI matches. If several usable PubChem CIDs matched the same ChEBI entity, we retained one CID to avoid duplication. This produced 42,068 ChEBI-to-PubChem mappings involving 40,656 PubChem CIDs. A role-based group contains the distinct mapped CIDs sharing the same direct ChEBI \texttt{has\_role} annotation and does not imply structural relatedness.

Of 1,313 roles with at least one available mapped entity, 597 had 1 to 3 members, 518 had 4 to 30, and 198 had more than 30 members. The primary cohort used the 518 groups with 4 to 30 available entities, containing 5,783 compound-to-role assignments and 4,494 unique entities. We required at least four compounds so that each role could be evaluated as a group. Roles with more than 30 compounds were evaluated separately because they were considerably broader.
...
With this approach we obtained a total of 1,554 queries. All 44,887 descriptions, were embedded using the baseline Nomic model, ChEmbed\textsubscript{vanilla}, and ChEmbed\textsubscript{progressive}. The resulting embeddings were then ranked against each query. The directly annotated CIDs formed the reference set for that role. Average precision summarizes how early and consistently these reference descriptions appear across the complete ranking, while recall@10 is the proportion of reference descriptions that appear among the first ten results.
...
\paragraph*{Findings }
Table~\ref{tab:chebi-role-retrieval} summarizes the results. ChEmbed\textsubscript{progressive} improved performance for 435 of the 518 primary role-based groups relative to Nomic, with a mean gain of $+0.139$ average precision (AP) per group (paired $t$-test across the 518 groups, 95\% CI $[0.124, 0.154]$, $p<0.001$). ChEmbed\textsubscript{vanilla} improved fewer groups, 298 out of 518, and by a much smaller margin. The improvement held across all three query templates, with gains of $+0.168$, $+0.112$ and $+0.137$ AP. A separate sensitivity analysis restricted to one-to-one PubChem-to-ChEBI correspondences produced an essentially identical model advantage, while including groups with more than 30 members preserved the same overall trend. Fig.~\ref{fig:role-retrieval-cohort}-A shows the average precision for each primary role-based group in ChEmbed\textsubscript{progressive} versus the base Nomic model, providing greater transparency than the aggregated AP values reported in Table~\ref{tab:chebi-role-retrieval}. Fig.~\ref{fig:role-retrieval-cohort}-B compares the mean AP of the three models under two conditions: the original PubChem descriptions, and a role-phrase masking control in which the exact queried ChEBI role label was removed from the reference descriptions before ranking. The masking result is discussed below.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/chebi-role-retrieval.pdf}
    \caption{Chemical-role retrieval across the 518-group primary cohort. (a) Group-level average precision (AP) for ChEmbed\textsubscript{progressive} versus Nomic; points above the diagonal had higher average precision in fine-tuned ChEmbed model compared to the base Nomic model. (b) Mean AP over the 518 groups, before and after masking the exact ChEBI role phrase in the descriptions. Error bars are 95\% bootstrap confidence intervals. Stars are paired $t$-tests against Nomic within each condition ($^{*}p<0.05$, $^{***}p<0.001$).}
    \label{fig:role-retrieval-cohort}
\end{figure*}

To assess how much explicit role wording contributed to the result, we repeated the evaluation after removing the exact queried ChEBI role label from the corresponding reference descriptions. That label appeared in 5,110 of the 5,783 (88\%) evaluated CID-to-role links. After masking it, average precision fell from 0.176 to 0.052 for Nomic and from 0.315 to 0.091 for ChEmbed\textsubscript{progressive}. Explicit role wording is therefore an important part of the retrieval signal. Fig.~\ref{fig:role-retrieval-cohort}-B illustrates these results. A smaller but still significant progressive advantage remained, but this control does not identify its mechanism. Because the role-member descriptions came from a PubChem source used during training, this analysis characterizes learned retrieval behavior rather than held-out chemical knowledge.
The most strongly supported interpretation is that ChEmbed, particularly the progressive variant, better aligns explicit chemical-role queries with relevant PubChem descriptions. It does not demonstrate inference from molecular structure or learning of a complete chemical ontology.

To illustrate this result in greater detail, Fig.~\ref{fig:factor-xa-retrieval} shows the eight PubChem records which are defined as coagulation factor Xa inhibitors by ChEBI. The corresponding query in the role-based retrieval evaluation dataset is: ``What chemicals can function as EC 3.4.21.6 (coagulation factor Xa) inhibitor?''. For this query, Nomic embedding retrieved 3 out of 8 directly annotated PubChem entities in its top 10 results; however, ChEmbed\textsubscript{progressive} retrieved 7 out of 8 PubChem entities within the top 10 results.
It should be noted that the eight PubChem records correspond to six inhibitor identities, since three of these eight entities belong to Edoxaban, which has three records: the free base, the tosylate salt, and the tosylate monohydrate. This example illustrates impact of fine-tuning at the level of individual compounds; however, the overall conclusions presented above are based on the complete set of 518 role-based groups.


\begin{figure*}[!t]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/factor-xa-retrieval.pdf}
...
\begin{itemize}
    \item \textbf{Model Weights:} The fine-tuned model weights for all ChEmbed variants are available on Hugging Face at \href{https://huggingface.co/collections/BASF-AI/chembed}{huggingface.co/collections/BASF-AI/chembed} for non-commercial use.
    \item \textbf{Training Data:} A subset of the training data is available at \href{https://huggingface.co/datasets/BASF-AI/ChEmbed-Training}{huggingface.co/datasets/BASF-AI/ChEmbed-Training}. A collection of the raw open-source data used to construct the training corpus is available at \href{https://huggingface.co/collections/BASF-AI/chembed-training-data}{huggingface.co/collections/BASF-AI/chembed-training-data}.
    \item \textbf{Evaluation Benchmark:} The ChemRxiv Retrieval benchmark introduced in this work is available at \href{https://huggingface.co/datasets/BASF-AI/ChemRxivRetrieval}{huggingface.co/datasets/BASF-AI/ChemRxivRetrieval}.
    \item \textbf{Supporting Materials:} The data collection and synthetic query-generation pipelines, evaluation code, raw evaluation results, analyses, expert-review materials, and scripts for the manuscript tables and generated figures are available at \href{https://github.com/HSILA/ChEmbed-Artifacts}{github.com/HSILA/ChEmbed-Artifacts}.
\end{itemize}

Permanent DOIs for the repository and datasets (via Zenodo) will be made available upon acceptance of the manuscript.

\section*{Acknowledgements}
The author(s) gratefully acknowledge the financial support received for the research, authorship, and/or publication of this article, made possible by \textbf{MITACS} funding number IT32409. This project also benefited from the computational resources provided by the Narval clusters of the \textbf{Digital Research Alliance of Canada}. We extend our sincere appreciation to \textbf{Adam Wojciech Bartwiki} for his project management contributions and to \textbf{Tobias Roth} for his support with computational resources and technical assistance. The authors acknowledge the use of large language models (Claude Opus 5, Claude 3.5 Sonnet, GPT-5, GPT-5.1, GPT-5.6 Sol, and Gemini 3.1) as coding assistants and for linguistic refinement during the preparation of this manuscript. The language models used to generate the training and evaluation queries are described in Section~\ref{subsec:query-generation}. All outputs were reviewed and verified by the authors, who take full responsibility for the content of this article.
